...
class ViewQuoteController extends GetxController {
  ...
  Future refreshHome({bool showMessage = false, String? statusId}) async {
    ...
    changeTab(statusId!);
    if (showMessage) {
      Get.showSnackbar(Ui.SuccessSnackBar(message: "Job page refreshed successfully".tr));
    }
  }
  ...
  Future refreshBooking({bool showMessage = false}) async {
    // await getBooking();
    if (showMessage) {
      Get.showSnackbar(Ui.SuccessSnackBar(message: "Quote page refreshed successfully".tr));
    }
  }
